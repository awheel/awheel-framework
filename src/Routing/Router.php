...
class Router
{
	...
	public function callAction($route)
	{
		...
		if ($action instanceof Closure) {
			return call_user_func_array($action, $route[2]);
		}
		...
		// 前置如果返回内容
		// todo 取消 __before
		if (method_exists($instance, '__before')) {
			call_user_func_array([$instance, '__before'], $route[2]);
		}

		// 方法未定义时调用, 可替换 missingMethod 为自己需要的
